...
export function WorkspaceHome({ api, overview, onOpenProject }) {
    ...
    <div className="px-5 py-6 lg:px-8 lg:py-8">
      <div className="grid gap-6 xl:grid-cols-[1fr_340px]">
        <section className="min-w-0">
          <div className="flex flex-col gap-4 border-b border-canopy-border pb-6 md:flex-row md:items-end md:justify-between">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.14em] text-canopy-green">Workspace</p>
              <h1 className="mt-3 text-3xl font-semibold text-white">Your creative work, with its history intact.</h1>
              <p className="mt-3 max-w-2xl text-sm leading-6 text-canopy-secondary">Continue recent projects, inspect creative versions, and pick up the thread where the work last changed.</p>
            </div>
            <div className="flex flex-wrap gap-2">
              <Button className="rounded-md bg-canopy-green text-[#06251A] hover:bg-[#54e7ad]" disabled={!name.trim() || createProject.isPending} onClick={() => createProject.mutate()}>
...
function QuickActions({ name, setName, creativeGoal, setCreativeGoal, createProject, latestProject, onOpenProject }) {
  return <aside className="border border-canopy-border bg-canopy-surface p-5">
    <h2 className="text-sm font-semibold text-white">Quick actions</h2>
    <p className="mt-2 text-sm leading-6 text-canopy-secondary">Create a project or jump into the most recent workspace context.</p>
    <div className="mt-5 space-y-3">
      ...
      <Button className="w-full rounded-md bg-canopy-green text-[#06251A] hover:bg-[#54e7ad]" disabled={!name.trim() || createProject.isPending} onClick={() => createProject.mutate()}>
        {createProject.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Plus className="mr-2 h-4 w-4" />}
        Create project
      </Button>
      {latestProject ? <Button variant="outline" className="w-full rounded-md border-canopy-border text-white" onClick={() => onOpenProject(latestProject.id)}><ArrowRight className="mr-2 h-4 w-4" />Open recent project</Button> : null}
    </div>
  </aside>;
}

function ProjectSearch({ value, onChange }) {
  return <div className="flex items-center gap-2 border border-canopy-border bg-canopy-surface px-3 py-2"><Search className="h-4 w-4 text-canopy-muted" /><input value={value} onChange={(event) => onChange(event.target.value)} placeholder="Search projects by name or goal" className="min-w-0 flex-1 bg-transparent text-sm text-white outline-none placeholder:text-canopy-muted" /></div>;
}
...
function RecentProjects({ projects, onOpenProject }) {
  return <Section title="Recent projects" description="Creative projects ordered by real project and version activity.">
    {projects.length ? <div className="divide-y divide-canopy-border border-y border-canopy-border">{projects.map((project) => <button key={project.id} className="group grid w-full gap-3 px-1 py-4 text-left transition hover:bg-canopy-surface sm:grid-cols-[1fr_auto]" onClick={() => onOpenProject(project.id)}>
      <div className="min-w-0">
        <h3 className="truncate text-base font-semibold text-white group-hover:text-canopy-green">{project.name}</h3>
        <p className="mt-1 line-clamp-2 text-sm leading-6 text-canopy-secondary">{project.creative_goal || 'No creative goal set'}</p>
...
function RecentVersions({ versions, onOpenProject }) {
  return <Section title="Recent versions" description="Latest creative changes across your projects.">
    {versions.length ? <div className="divide-y divide-canopy-border">{versions.map((version) => <button key={version.id} className="flex w-full items-start gap-3 py-3 text-left hover:bg-canopy-surface" onClick={() => onOpenProject(version.project.id)}>
      <div className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center border border-canopy-border bg-canopy-bg text-canopy-green"><GitCommit className="h-4 w-4" /></div>
...
function WorkspaceEmptyState({ name, setName, creativeGoal, setCreativeGoal, createProject }) {
  return <div className="mt-8 border border-canopy-border bg-canopy-surface p-8"><EmptyState icon={<Upload className="h-8 w-8" />} title="Your creative workspace is empty." description="Create your first project to start preserving creative history." /><div className="mx-auto mt-6 grid max-w-xl gap-3"><Input value={name} onChange={(event) => setName(event.target.value)} placeholder="Project name" /><Textarea value={creativeGoal} onChange={(event) => setCreativeGoal(event.target.value)} placeholder="Creative goal" className="h-20" /><Button className="rounded-md bg-canopy-green text-[#06251A] hover:bg-[#54e7ad]" disabled={!name.trim() || createProject.isPending} onClick={() => createProject.mutate()}>{createProject.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Plus className="mr-2 h-4 w-4" />}Create project</Button></div></div>;
}

function WorkspaceError({ onRetry }) {
  return <div className="px-5 py-10 lg:px-8"><div className="max-w-xl border border-rose-500/30 bg-rose-500/10 p-6"><AlertCircle className="h-6 w-6 text-rose-300" /><h1 className="mt-4 text-xl font-semibold text-white">Could not load your workspace.</h1><p className="mt-2 text-sm leading-6 text-rose-100">We could not retrieve your latest projects and activity.</p><Button className="mt-5 rounded-md bg-canopy-green text-[#06251A] hover:bg-[#54e7ad]" onClick={onRetry}><RefreshCw className="mr-2 h-4 w-4" />Try again</Button></div></div>;
}

function WorkspaceSkeleton() {
  return <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.1fr)_minmax(320px,0.9fr)]"><div className="space-y-3">{[0, 1, 2].map((item) => <div key={item} className="h-24 animate-pulse border border-canopy-border bg-canopy-surface" />)}</div><div className="space-y-3">{[0, 1, 2, 3].map((item) => <div key={item} className="h-16 animate-pulse border border-canopy-border bg-canopy-surface" />)}</div></div>;
}

function Section({ title, description, children }) {
  return <section><div className="mb-3"><h2 className="text-sm font-semibold uppercase tracking-[0.14em] text-canopy-muted">{title}</h2><p className="mt-1 text-sm leading-6 text-canopy-secondary">{description}</p></div>{children}</section>;
}
