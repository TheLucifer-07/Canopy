...
export class PostgresCoreRepository {
  ...
  async createSignedAssetUrl({ ownerId, assetId }) { const asset = one(await this.pool.query('SELECT a.* FROM assets a JOIN project_assets pa ON pa.asset_id=a.id JOIN projects p ON p.id=pa.project_id WHERE a.id=$1 AND p.owner_id=$2 LIMIT 1', [assetId, ownerId])); if (!asset) throw new ApiError(ERROR_CODES.ASSET_NOT_FOUND, 'Asset not found.', { statusCode: 404 }); return { asset_id: asset.id, url: `${config.storage.publicBaseUrl}/assets/${asset.id}/content`, expires_in: 600 }; }
  async downloadAssetBytes({ ownerId, assetId }) { const asset = one(await this.pool.query('SELECT a.* FROM assets a JOIN project_assets pa ON pa.asset_id=a.id JOIN projects p ON p.id=pa.project_id WHERE a.id=$1 AND p.owner_id=$2 LIMIT 1', [assetId, ownerId])); if (!asset) throw new ApiError(ERROR_CODES.ASSET_NOT_FOUND, 'Asset not found.', { statusCode: 404 }); return { asset, bytes: await readFile(join(this.storageRoot, asset.storage_key)), mime: asset.mime }; }
  ...
  async searchCopilotVersions({projectId,ownerId,query='',limit=8}) { const l=(await this.getLineage({projectId,ownerId})).versions; const q=String(query).toLowerCase(); return l.filter(v=>!q||JSON.stringify(v).toLowerCase().includes(q)).slice(-Math.min(Number(limit)||8,50)); }
  async searchCopilotMemories({projectId,ownerId,query='',types=null,limit=8}) { const l=await this.listMemories({projectId,ownerId}); const terms=searchTerms(query), allowed=Array.isArray(types)?new Set(types):null; return l.filter(m=>{ const text=JSON.stringify(m).toLowerCase(); return (!allowed||allowed.has(m.type))&&(!terms.length||terms.some(term=>text.includes(term))); }).slice(0,Math.min(Number(limit)||8,50)); }
